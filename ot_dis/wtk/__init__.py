...
def transform_to_dist_matrix(time_series_train, time_series_test, k, normalized=False):
    '''
    Computes the distance matric6es for training and test.
...
def pairwise_subsequence_kernel(
    ...
    for i, ts_i in  tqdm(enumerate(time_series_train), total=len(time_series_train), desc='Processing'):
        for j, ts_j in  tqdm(enumerate(time_series_train[i:]), total=len(time_series_train[i:]), desc='Processing'):
        ...
        for j, ts_j in  tqdm(enumerate(time_series_test), total=len(time_series_test), desc='Processing'):
            s_i = subsequences_train[i] # first shapelet
...
def wtk_distance_fix(a, b):
    ...
    subs_b = subsequences(b, 25)
    dis = wasserstein_kernel_fix(subs_a, subs_b)
    return np.exp(-gamma * dis)

def wasserstein_kernel_fix(subsequences_1, subsequences_2, metric='euclidean'):
    '''
    Calculates the distance between two time series using their
    corresponding set of subsequences. The metric used to align
    them may be optionally changed.
